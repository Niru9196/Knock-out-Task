    ...
    return (
      ...
      >
        <div className="flex justify-between items-center border-b pb-2">
          <span className="text-lg font-semibold text-gray-800">{task.taskTitle}</span>
          <EditTaskDropdown
            id={task.id}
        ...
        <div className="text-gray-600 mt-2">{removeHtmlTags(task.description)}</div>
        <div className="mt-3 text-blue-500 font-medium">{capitalizeFirstLetter(task.taskCategory)}</div>
      </div>
    );
  };
