  ...
  const signupUser = useCallback(
    async ({ email, password }: SignupFormValues) => {
      try {
        ...
        if (!userCredential) {
          return message.error("Error in signing up");
        }
        navigate("/");
      } catch (error: any) {
        ...
        message.error(
          errorMessages[error.code] || "Signup failed, please try again.",
        );
      }
    },
    [auth, navigate],
    ...
    <div className="flex justify-center items-center min-h-screen bg-gray-100 px-4 relative">
      ...
      <Card className="w-full max-w-md shadow-lg rounded-xl p-6 bg-white relative z-10">
        <div className="flex items-center justify-center gap-2 mb-4">
          <RedEnvelopeOutlined style={{ color: "#7B1984", fontSize: "28px" }} />
        ...
        <h1 className="text-center text-2xl font-bold -mt-3 mb-3">Sign up</h1>
